...
data_map_regions <- data %>% filter(Year == "2016")

# On récupère la moyenne des scores de bonheur pour chaque région pour l'année 2016

df2 <- data %>% 
  filter(Year == "2016") %>%
  select(Year, Happiness_score, Region, Geometry) %>%
...
mypalette <- colorNumeric(
  palette="BuPu", 
  domain=df2$Happiness_score, 
  na.color="transparent")

map_regions <- leaflet() %>%
  addTiles() %>%
  # polygone des pays
  addPolygons(data = data_map_regions$Geometry,
              label = data_map_regions$Region,
              fillColor = mypalette(df2$Happiness_score),
              smoothFactor = 1,
              weight = 2,
              opacity = 1,
              color = "white"
