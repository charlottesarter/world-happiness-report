...
world_factors_ranking_graph <- data %>%
  ...
  ggplot(aes(y = reorder(Factor_name, Factor_value), x = Factor_value)) + 
...
ggplotly(world_factors_ranking_graph)
```
...
factors_per_region_graph <- data %>%
  ...
  ggplot(aes(x = reorder(Factor_name, desc(Factor_value)), y = Factor_value, fill = reorder(Factor_name, desc(Factor_value)))) +
    ...
    theme(axis.title.x=element_blank(), 
        axis.text.x=element_blank(), 
        axis.ticks.x=element_blank()) +
    labs(title = "Importance des différents facteurs de bonheur par région en 2021", fill = "Facteurs", y = "Valeur")
