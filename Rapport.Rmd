...
Contrairement à la visualisation précédante, on constate que la majorité des pays de ce Top 10 n'appartienent pas aux deux régions les mieux classées (sans compter la *Nouvelle Zélande* qui occupe la 9ème position du classement). En effet, **9 des 10 pays du classement font partie de l'*Europe de l'Ouest***, pourtant elle n'occupe que la 3ème place du classement par région. Cela montre qu'il doit y avoir des disparités entre les pays de cette région.
...
L'intérêt principal de ce graphique est de constater en un coup d'oeil que **les scores moyens par région restent très stables**. Globalement, chaque région du monde conserve un score de bonheur similaire, est l'allure globale de ces graphes évolue peu au cours des années.
...
Ce diagramme en barres permet de visualiser le score de bonheur le plus présent dans une région. Le score de chaque pays est arrondi à l'entier le plus proche. Le graphique prend donc compte de chacun des scores de bonheur individuel de chaque pays, puis les regroupe par régions.
...
generosity_correlation_graph <- data %>%
  ...
  # se = FALSE,
  # size = 1) +
  labs(title = "Score de bonheur en fonction du niveau de générosité", 
...
# data %>%
#       group_by(Year) %>%
#       summarise(Explained_by_generosity = mean(Explained_by_generosity),
#                 Explained_by_life_expectancy = mean(Explained_by_life_expectancy),
#                 Explained_by_freedom = mean(Explained_by_freedom),
#                 Explained_by_social_support = mean(Explained_by_social_support),
#                 Explained_by_trust_government = mean(Explained_by_trust_government))
