...
data_map_countries <- data %>% filter(Year == "2021")
...
Le graphique qui suit montre la corrélation éventuelle entre le score de bonheur et le **niveau de pollution** de la ville. 

> Les villes sont colorées en fonction de la région du monde où elles sont situées.


```{r corrélation pollution}
...
On constate une corrélation assez claire entre les deux facteurs. **Plus le niveau de pollution est faible, plus le score de bonheur des populations est élevé**. Le niveau de pollution semble donc être un facteur impactant le score de bonheur.
...
Le dataset complémentaire propose également une varaible qui correspond au prix d'une bouteille d'eau dans les différentes villes. A nouveau si on compare ce chiffre avec le score de bonheur de la ville en question nous pouvons voir une corrélation assez surprenante.
