...
3. Les Top 10 des pays dont le score de bonheur a le plus augmenté / diminué entre 2015 et 2021

4. L'évolution du score de bonheur moyen des régions chaque année entre 2015 et 2021 (bar chart)
...
![](data/rshiny_happiness_score_countries.png)
...
1.  Un boxplot qui indique la distribution des scores de bonheur de chaque région chaque année
...
4.  Les cartes réalisées avec **Tableau** : la contribution des différents facteurs au score de bonheur
...
C'est une des visualisations les plus attendues au vu de notre jeu de données : **une carte** ! Rien de mieux pour visualiser et situer géographiquement les pays et régions les plus heureux. 

> Cette carte a été réalisée avec le package *Leaflet* de R. Elle est **interactive** : lorsque vous survolez un pays avec votre souris, le nom du pays, son classement et son score de bonheur précis s'affichent.
...
En un coup d'oeil, on visualise directement les pays et régions les plus heureux : ce sont ceux les plus foncés sur la carte, soit l'*Amérique du Nord*, l'*Europe du Nord* et de l'*Ouest* et l'*Australie et la Nouvelle-Zélande*. 
...
### 4. Les cartes réalisées avec **Tableau** : un aperçu de la contribution des facteurs au bonheur

Les cartes suivantes ont toutes été réalisées sur **Tableau**, et intégrées au rapport via des fichiers .png trouvables dans le dossier *\_data*. L'intégralité de nos réalisations sur Tableau sont également disponibles dans le fichier "visualisations_tableau.twb".  

La carte suivante permet de visualiser la **contribution du support social au score de bonheur dans le monde**, moyennée de 2015 à 2021 :
<br>
![](data/contribution_support_social.png)
<br>
On constate globalement que le **support social contribue énormément au bonheur dans beaucoup de pays**. Nous essaierons de voir si cette tendance se confirme à l'aide d'un graphique hiérarchisant la contribution de chaque facteur au bonheur.  
<br>
Nous pourrions penser que la **liberté des individus** est un élément essentiel à leur bonheur. Voyons dans quelle mesure ce facteur y contribue :
<br>
![](data/contribution_liberte.png)
<br>
On constate cette fois des **niveaux de contribution plus faibles**, bien qu'existants. Notamment, le contraste en Russie avec la précédente carte est saisissant, mais finalement relativement logique au vu du régime politique en place dans ce pays...
...
1. La corrélation entre le score de bonheur des pays et chacun des facteurs étudiés

2. L'analyse de facteurs supplémentaires 
...
Ce facteur sont donc a priori moins impactant sur les variations de score entre les différents pays.
...
Le graphique qui suit montre la corrélation éventuelle entre le score de bonheur et le niveau de pollution d'un ville. Les villes sont rassemblées par régions du monde.
...
> En réalité, pour pousser notre analyse, il faudrait étudider la **corrélation entre le niveau de pollution et le niveau de développement des pays**. En effet, si on constate que les pays les plus pollués sont les moins développés, on ne pourrait pas affirmer que la pollution influe sur le score de bonheur des pays.

Nous avons donc tracé le **graphique de corrélation entre le facteur de pollution et le niveau de développement** (PIB) des pays.


```{r corrélation PIB pollution}

pollution_pib_correlation_graph <- data2021_complement %>%
  ggplot(aes(x = Pollution_score, 

             y = Logged_gpd_per_capita, text = paste("Country: ", Country, "\n", "City: ", City, "\n", "Rank: ", Happiness_rank))) + 
  geom_point(aes(color = Regional_indicator)) + 
  geom_smooth(inherit.aes = FALSE, ## on est obligé de redéfinir l'aes ici à cause du text = paste dans l'aes ggplot
              aes(x= Pollution_score, y = Logged_gpd_per_capita),
              #method = "lm",
              col = "#C42126",
              se =FALSE,
              size = 1) + 
  labs(title = "Corrélation entre le niveau de pollution et le PIB par habitant", 
       x = "Niveau de pollution", 
       y = "Log(PIB par habitant)")

pollution_pib_correlation_graph %>% ggplotly(tooltip = "text")
```

Le graphique ci-dessus montre qu'il existe bien une corrélation entre le niveau de pollution et le PIB par habitant. En effet, **les pays ayant un PIB élevé sont ceux ayant un score de pollution plus faible**. 

Cette découverte paraît étrange au vu de ce que l'on sait : la Chine et les États-Unis sont les pays qui polluent le plus. Cependant, ce n'est visiblement pas aux États-Unis que se fait le plus ressentir la pollution. Ainsi, les pays développés ont bien un niveau de pollution plus faible que les pays en voie de développement, ce qui ne permet pas vraiment de tirer une conslusion de notre analyse précédente. 

En effet, il paraît aussi assez plausible que les pays en voie de développement ou sous-développés ont d'autres préoccupations plus importantes que leur impact environnemental, ce qui peut expliquer le phénomène observé.


Le dataset complémentaire propose également une variable qui correspond au **prix d'une bouteille d'eau** dans les différentes villes. À nouveau, si on compare ce chiffre avec le score de bonheur des villes, nous pouvons voir une corrélation assez surprenante.

Les lieux où le prix de l'eau est le plus faible sont les lieux avec le score de bonheur le plus faible.

On peut alors penser que plus le prix de l'eau est élevé, plus le pays est riche et développé. Cela rejoint notre remarque précédente : **le score de bonheur est bien corrélé au niveau de développement des pays, mais pas forcément au prix de la vie ou au niveau de pollution**, qui sont des conséquences du niveau de développement.
...
water_correlation_graph <- data2021_complement  %>%
  ...
  geom_smooth(inherit.aes = FALSE, ## on est obligé de redéfinir l'aes ici à cause du text = paste dans l'aes ggplot
              ...
              se =FALSE,
              size = 1) + 
  labs(title = "Corrélation entre le prix d'une bouteille d'eau et le score de bonheur", 
       x = "Prix d'une bouteille d'eau (£)", 
       y = "Score de bonheur")
...
Afin de vérifier l'observation précédente, il peut être interessant d'observer la **corrélation entre le PIB par habitant et le prix d'une bouteille d'eau**.
...
pib_water_correlation_graph <- data2021_complement  %>%
  ...
  geom_smooth(inherit.aes = FALSE, ## on est obligé de redéfinir l'aes ici à cause du text = paste dans l'aes ggplot
              ...
              se =FALSE,
              size = 1) + 
  labs(title = "Corrélation entre le prix d'une bouteille d'eau et (bite) le PIB par habitant", 
       x = "Prix d'une bouteille d'eau (£)", 
       y = "Log(PIB par habitant)")
...
Le graphique est en accord avec ce qui avait été observé. En effet, plus le PIB est élevé, plus le prix d'une bouteille d'eau augmente, ce qui montre une **corrélation entre le prix d'e l'eau'une bouteille d'eau et le niveau de développement des pays**.

> Ici, nous étudions le prix d'une bouteille d'eau, mais celui-ci reflète d'une manière plus générale le **prix de la vie** dans le pays.


# V) Quels sont les facteurs qui contribuent le plus au bonheur ?

Notre dernier axe d'analyse concerne le **poids des différents facteurs dans les scores de bonheur**. Nous allons répondre aux questions suivantes : 

- Est-ce que les facteurs contribuent tous autant au bonheur ?

- Est-ce que les facteurs qui contribuent le plus au bonheur sont les mêmes dans toutes les régions du monde ?

## Les visualisations réalisées

1. Le classement des facteurs en fonction de leur contribution au bonheur mondial

2. Le classement des facteurs en fonction de leur conribution au bonheur dans les différentes régions du monde (2021)

## Les facteurs qui contribuent le plus au bonheur dans le monde depuis 2015

Nous avons voulu connaître l'**influence de chaque facteur au niveau mondial**. Le graphique ci-dessous regroupe toutes les régions et toutes les années, et donne ainsi la valeur moyenne de la contribution des différents facteurs au bonheur.


```{r classement facteurs}

world_factors_ranking_graph <- data %>%
...
world_factors_ranking_graph %>% ggplotly(tooltip = "text")
```

En moyenne et sur les 6 dernières années, c’est le **facteur social qui contribue le plus au bonheur**, suivi de l’**économie**. L'entourage d'une personne est donc un facteur universellement corrélé au bonheur des populations. 

La **confiance dans le gouvernement** arrive en dernier dans ce classement. Cela confirme l'observation faite dans la partie précédente. En effet, la perception de corruption est assez élevée pour tout le monde, et ne contribue pas vraiment à rendre les gens heureux...Il est donc cohérent que la confiance dans le gouvernement ne soit pas un facteur d’influence important.

## La contribution des facteurs au bonheur dans les différentes régions du monde (2021)

Nous avons voulu préciser l'analyse obtenue via le précédent graphique, en nous demandant quelles étaient les tendances de contribution des facteurs **dans chaque région** en 2021. Plusieurs constats nous ont alors frappé :  

-   Dans les régions développées, qui sont aussi globalement les plus heureuses, la tendance de répartition de la contribution des facteurs reste semblable.

-   Dans les régions les moins heureuses, le niveau de contribution au bonheur des différents facteurs est plus bas (ce qui est logique), et le rôle de chacun des facteurs est plus équilibré.

-   Enfin, et c'est le constat le plus important et l'observation la plus intéressante, on remarque un constraste non négligeable entre le précédent graphique et celui-ci. En effet, le graphe précédent place le **support social** comme facteur contribuant le plus au bonheur **en moyenne** de 2015 à 2021. Cependant, on constate ici que c'est globalement le **facteur économique** et non social qui prédomine en 2021. Ce phénomène peut aisément s'expliquer par la **pandémie** qui a frappé la plupart des régions du monde en 2020, et les confinements qui en ont résulté. Les interactions sociales entre les gens ont alors été grandement réduites, ce qui explique leur importance moindre dans le score de bonheur de 2021.
...
factors_per_region_graph <- data %>%
  ...
  ggplot(aes(x = reorder(Factor_name, desc(Factor_value)), 
             y = Factor_value, 
             fill = reorder(Factor_name, desc(Factor_value)),
             text = paste("Factor value: ", round(Factor_value,3)))) +
    geom_bar(stat = "identity") +
    ...
    theme(axis.title.x = element_blank(), 
        axis.text.x = element_blank(), 
        axis.ticks.x = element_blank()) +
    labs(title = "Importance des différents facteurs de bonheur par région en 2021", 
         fill = "Facteurs", 
         y = "Valeur")

factors_per_region_graph %>% ggplotly(tooltip = "text")

```

# Conclusion

En conclusion, le dataset du **World Happiness Report** nous a permis d’étudier le niveau de bonheur à travers le monde. Nous avons d'abord **constaté** puis **analysé** les **variations temporelles et géographiques** du socre de bonheur, puis nous avons tenté de **mettre en lumière les raisons de ces variations** en **contextualisant** au maximum les données.

Nous avons débuté notre analyse en établissant le **classement des différentes régions du monde en fonction de leur score de bonheur**. Sans trop de surprise, les **régions les plus développées arrivent en tête du classement**, et inversement, les régions les moins développées arrivent en bas du classement. Les régions ayant un score de bonheur plus faible sont aussi souvent des régions où l’**atmosphère globale est assez médiocre** (conflits, climat dangereux, insécurité...). 

Ensuite, nous avons analysé l'**évolution du score de bonheur dans chacune des régions du monde depuis 2015**. Nous avons observé que les pays voyant leur score de bonheur augmenter ces dernières années étaient, pour la plupart, des pays en bas du classement. Le Bénin notamment, est le pays dont le score de bonheur a le plus augmenté ces dernières années. Nous nous sommes alors posé la question de la **pertinence de l'échantillon d'individus intérrogé pour l'enquête** puisque la situation actuelle au Bénin ne semble pas aller de pair avec un climat “favorable” à une hausse du score de bonheur. 

Cette observation nous a également amené à nous questionner sur les buts du World Happiness report. Cette étude contribue au **Réseau des solutions de développement durable des Nations unis**. Elle permettrait de donner une plus grande visibilité aux recherches qui analysent la construction et les moyens de favoriser le bonheur à travers le monde. 

En effet, le climat global d'un pays peut être étudié par le biais de **différents facteurs, dont nous avons étudié l'impact sur les scores de bonheur**. Notre analyse factorielle a confirmé certaines de nos idées préconçues : le niveau de liberté, le PIB par habitant ou encore le niveau de soutien social sont fortement corrélés au niveau de bonheur. D’autres données pourraient sûrement nous apporter plus d'informations quant aux facteurs influant sur le niveau de bonheur. Par exemple, les habitants de la Finlande (premier pays du classement) ne justifient pas seulement leur niveau de bonheur par les facteurs étudiés. En effet, beaucoup l'expliquent par la **qualité de l'environnement** (espaces verts, réseau de transport fiable, bon niveau d'éducation...). 

### Notre avis sur le projet

> "J'ai trouvé ce projet très intéressant et enrichissant. J'ai pu découvrir deux logiciels, Tableau et RStudio, et un nouveau langage de programmation dont l'utilité diffère des langages que je connais déjà comme le Java ou le C. J'ai trouvé que le R est un langage qui peut être difficile à débugger, ainsi j'ai beaucoup appris à mieux formuler mes problèmes pour être plus efficace lors de mes recheches de solutions sur internet. J'aurais aimé plus développer la **création de cartes interactives avec Leaflet**, notamment l'idée que j'avais en tête d'introduire un graphique dynamique dans les info-bulles des pays tracés sur la carte." *Charlotte* 

> "Ce projet m’a permis de découvrir le langage R par le biais d’une étude assez libre. J'ai eu du mal par moment à aller au bout des graphiques que j'avais en tête. J’aurais aimé que l’on puisse centrer une partie de l’étude sur un pays en particulier afin de voir les disparités à l’échelle d’un pays. Malheureusement, la plupart de nos données sont à plus grandes échelles et ne permettaient donc pas cette étude." *Maud*

> "Me destinant à un métier de data analyst, cette UE m'a permis de découvrir le langage R, puissant et simple d'utilisation dans une optique de faire des dashboard. J'ai notamment beaucoup apprécié la partie Rshiny dans cette optique, et aurais aimé l'approfondir un peu plus. De plus, la partie du cours s'attardant sur la bonne façon de représenter une donnée, et les différents biais inhérents à certaines méthodes de représentation apporte une réelle plus-value." *Evann*
