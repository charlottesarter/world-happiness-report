...
mean_hs_score_world <- data %>% 
  ...
  ggplot(aes(x=Year, y=Happiness_score)) + geom_line(stat = "identity", color = "steelblue") + labs(title = "Evolution du score de bonheur moyen dans le monde", x = "Année", y = "Score de bonheur (moyenne monde)")
...
plot_positive_progress <- df_diff %>%
  ggplot(aes(x = reorder(Country, desc(Score_progression)), y = Score_progression)) + geom_bar(stat = "identity", fill = greens(10)) + labs(title = "Les 10 pays dont le score a le plus progressé de 2015 à 2021", x = "Pays", y = "Pourcentage de progression")  + scale_y_continuous(labels = scales::percent) + theme(legend.position = "none") 
...
# Histogramme global année 2016 Maud
...
region_ranking_graph <- data %>%
  ...
  group_by(Region) %>%
  summarise(Happiness_score_mean = mean(Happiness_score)) %>%
  ggplot(aes(x = Happiness_score_mean, y = reorder(Region, Happiness_score_mean), fill = reorder(Region, desc(Happiness_score_mean)))) + 
  geom_bar(stat = "identity") +
  scale_fill_viridis_d() + 
  labs(title = "Classement des régions (année 2021)", x = "Score", y = "Région") +
  theme(legend.position = "none")

ggplotly(region_ranking_graph)
```

### X) Corrélation entre les différents facteurs et le score de bonheur
...
# On construit ce que l'on veut afficher lorsque l'on passe la souris sur un point 

mytext <- paste(
      "Country: ", data$Country,"<br/>", 
      "Rank: ", data$Happiness_rank, "<br/>",
      "Happiness score: ", data$Happiness_score, "<br/>",
...
freedom_correlation_graph <- data %>%
  filter(Year == 2021) %>%
  ggplot(aes(x = Freedom, y = Happiness_score, text = paste("Country: ", Country, "<br/>", "Score: ", Happiness_score) %>% lapply(htmltools::HTML))) + 
  ...
  geom_smooth(inherit.aes = FALSE, ## on est obligé de redéfinir l'aes ici à cause du text = paste dans l'aes ggplot
              ...
              se = FALSE,
              size = 1) + 
  labs(title = "Corrélation entre la liberté et le bonheur (2021)", x = "Niveau de liberté", y = "Score de bonheur")
...
gpd_correlation_graph <- data %>%
  filter(Year == 2021) %>%
  ggplot(aes(x=Logged_gpd_per_capita, y=Happiness_score)) + geom_point() + geom_smooth() + labs(title = "Corrélation entre PIB par habitant et bonheur", x = "Log(PIB par habitant)", y = "Score de bonheur")

gpd_correlation_graph %>% ggplotly()


life_expectancy_correlation_graph <- data %>%
  filter(Year == 2021) %>%
  ggplot(aes(x=Life_expectancy, y=Happiness_score)) + geom_point() + geom_smooth() + labs(title = "Corrélation entre espérance de vie et bonheur", x = "Espérance de vie", y = "Score de bonheur")

life_expectancy_correlation_graph %>% ggplotly()


social_support_correlation_graph <- data %>%
  filter(Year == 2021) %>%
  ggplot(aes(x=Social_support, y=Happiness_score)) + geom_point() + geom_smooth() + labs(title = "Corrélation entre soutien social et bonheur", x = "Niveau de soutien social", y = "Score de bonheur")

social_support_correlation_graph %>% ggplotly()


```

On peut une fois encore observer une corrélation générale entre ces derniers facteurs et le score de bonheur. Cependant, chacun des facteurs fournis dans ce dataset ne contribue par nécessairement grandement au score de bonheur. En effet, voici les graphiques de corrélation obtenus pour les facteurs "générosité" et "confiance dans le gouvernement":

```{r}
generosity_correlation_graph <- data %>%
  filter(Year == 2021) %>%
  ggplot(aes(x=Generosity, y=Happiness_score)) + geom_point() + geom_smooth() + labs(title = "Corrélation entre générosité et bonheur", x = "Niveau de générosité", y = "Score de bonheur")

generosity_correlation_graph %>% ggplotly()


trust_government_correlation_graph <- data %>%
  filter(Year == 2021) %>%
  ggplot(aes(x=Trust_government, y=Happiness_score)) + geom_point() + geom_smooth() + labs(title = "Corrélation entre confiance dans le gouvernement et bonheur", x = "Niveau de confiance dans le gouvernement", y = "Score de bonheur")

trust_government_correlation_graph %>% ggplotly()
```
